...
6. **Beautiful template integration** — When `beautiful-html-templates/` exists, use it as the preferred concrete template library for new decks and PPT conversions. Read its `AGENTS.md` operating manual and `index.json` before selecting templates.
...
Read [STYLE_PRESETS.md](STYLE_PRESETS.md) for safe preset candidates. If [beautiful-html-templates/](beautiful-html-templates/) exists, read [beautiful-html-templates/AGENTS.md](beautiful-html-templates/AGENTS.md) and [beautiful-html-templates/index.json](beautiful-html-templates/index.json), then use that library as the preferred source for the 3 visual template previews. If [bold-template-pack/selection-index.json](bold-template-pack/selection-index.json) exists, read that compact index too, but do not read any `design.md` files yet.
...
- When `beautiful-html-templates/` is available, generate 3 previews by default from three different `beautiful-html-templates/templates/*` candidates selected by tone, mood, formality, density, and occasion in `index.json`.
- If the beautiful-template matches are weak, generate 2 beautiful-template previews and 1 fallback preview from `STYLE_PRESETS.md`, `bold-template-pack`, or a custom wildcard.
- If `beautiful-html-templates/` is not available, generate 3 previews by default: 1 safe preset from `STYLE_PRESETS.md`, at least 1 bold template from `bold-template-pack/selection-index.json`, and 1 wildcard.
...
**Beautiful HTML template selection rules:**

- Follow [beautiful-html-templates/AGENTS.md](beautiful-html-templates/AGENTS.md) for template matching and adaptation, except adapt macOS-only `open <path>` instructions to the current environment's browser-opening workflow.
- Read `beautiful-html-templates/index.json` first. Match the user's occasion, mood/vibe, density, formality, and scheme against each template's `mood`, `tone`, `best_for`, `avoid_for`, `formality`, `density`, and `scheme`.
- Pick three candidates that feel genuinely different. Do not choose three variations of the same visual voice.
- For each candidate preview, read only that template's `template.html` plus `template.json` if needed. Use the first/cover slide as the preview and replace placeholder text with the user's real topic/title/subtitle/author/date.
- Save previews to `.claude-design/slide-previews/` using descriptive filenames such as `style-a-<slug>.html`.
- Keep the template's fonts, palette, decorative vocabulary, spacing rhythm, navigation runtime, and component grammar intact. Preview text must be real deck content, not template metadata.
- Template names, slugs, and file paths belong in the assistant message only, never on the slide canvas.
...
If the user selected a template from `beautiful-html-templates`, read that template's `template.html`, `template.json`, and `design.md` if present, plus [beautiful-html-templates/AGENTS.md](beautiful-html-templates/AGENTS.md). Treat the selected template as a closed visual system:

- Clone the chosen template folder into the user's output workspace before adapting it.
- Replace placeholder headlines, body copy, numbers, names, dates, section labels, chrome text, and image placeholders with the user's real content.
- Preserve the template's fonts, palette, CSS variables, layout grid, slide-level classes, decorative elements, navigation runtime, and spacing rhythm.
- Add slides by duplicating the closest existing layout. Remove unneeded slides from the bottom. Update page-number labels.
- If a needed layout is missing, design it from scratch using the same visual system; do not import layouts from another template.
- Keep the final deck on a fixed 16:9 stage. If the template uses `deck-stage.js`, keep it as a sibling runtime or inline it only if that is safer for delivery.
- For beautiful-template decks, a folder-based output with `template.html` plus local runtime/assets is allowed; it does not need to be collapsed into a single HTML file if doing so would break the template's runtime or asset paths.
- Verify the final deck in a browser and check for text overflow, panel overlap, and broken local assets.
...
- Single self-contained HTML file, all CSS/JS inline, except selected `beautiful-html-templates` decks may remain folder-based with local sibling runtime/assets
...
3. **Style selection** — Proceed to Phase 2 for style discovery. If `beautiful-html-templates/` exists, use it as the preferred template source for the conversion previews.
...
| [beautiful-html-templates/AGENTS.md](beautiful-html-templates/AGENTS.md) | Operating manual for selecting and adapting beautiful HTML templates | Phase 2 and Phase 3 when template library exists |
| [beautiful-html-templates/index.json](beautiful-html-templates/index.json) | Compact metadata for beautiful HTML template matching | Phase 2 (style selection) |
| [beautiful-html-templates/templates/*](beautiful-html-templates/templates/) | Full HTML template folders with design systems and runtimes | Phase 2 previews and Phase 3 final generation |
